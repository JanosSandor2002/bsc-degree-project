...
export const AppReducer = (state: typeof initialState, action: Action) => {
  switch (action.type) {
    ...
    case 'LOGOUT':
      return { ...state, user: null, token: null, projects: [], tasks: [], subtasks: [], selectedProject: null, sprints: [] };
    case 'SET_PROJECTS':
    ...
    case 'UPDATE_SUBTASK':
      return { ...state, subtasks: state.subtasks.map((s) => s._id === action.payload._id ? action.payload : s) };
    case 'DELETE_SUBTASK':
      return { ...state, subtasks: state.subtasks.filter((s) => s._id !== action.payload) };
    case 'SET_LOADING':
