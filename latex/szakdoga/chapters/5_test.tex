...
%
% Megjegyzés: egyes kódrészletek hibakeresése és javítása
% Claude (Anthropic) MI-alapú eszköz segítségével történt,
% minden esetben kritikus szakmai felülvizsgálattal párosulva.
\Chapter{Tesztelési stratégia és eredmények}
...
\begin{itemize}
  ...
  \item A projektek listázása csak a saját projekteket tartalmazza (más felhasználóé nem jelenik meg).
...
\textbf{logService tesztek}: A tesztek az in-memory adatbázison ellenőrzik, hogy a \texttt{logEvent} hívása után a bejegyzés valóban létrejön-e a \texttt{Log} kollekcióban, a \texttt{message} mező helyesen tárolódik-e, és az opcionális \texttt{userId} paraméter esetén a \texttt{user} mező is perzisztálódik-e.
...
A frontend tesztek Vitest \cite{vitest} tesztelési keretrendszert és React Testing Library-t alkalmaznak. A Vitest Vite-alapú, és natívan kezeli a TypeScript-et \cite{typescript} és a JSX-et. A React Testing Library a komponensek felhasználói szemszögből való tesztelésére ösztönöz: nem az implementáció részleteit, hanem a látható és interakcióra képes elemeket ellenőrzi.

A tesztek konfigurációja a Vitest konfigurációs fájlban a \texttt{jsdom} tesztkörnyezetet alkalmazza, a \texttt{@testing-library/jest-dom} pedig bővíti a matchers listát (pl.\ \texttt{toBeIn\-The\-Document}, \texttt{toHaveProperty}).

\SubSection{Reducer tesztek}

A reducer tesztek a legegyszerűbbek: tiszta függvényeket tesztelnek, amelyeknek nincs mellékhatásuk. Az \texttt{AppReducer} tesztjei tíz action típust ellenőriznek: \texttt{SET\_SUBTASKS}, \texttt{SET\_LOADING}, \texttt{SET\_SELECTED\_PROJECT}, \texttt{SET\_USER}, \texttt{LOGOUT}, \texttt{ADD\_TASK}, \texttt{DELETE\_TASK}, \texttt{ADD\_SPRINT}, \texttt{DELETE\_SPRINT} és \texttt{APPEND\_SUBTASKS}. A \texttt{ViewReducer} tesztjei verifikálják az ismeretlen action típusra adott alapértelmezett viselkedést, és az összes \texttt{ViewState} értékkel való kompatibilitást.
...
\textbf{CreateProject komponens}: A tesztek verifikálják a kezdő lépés megjelenítését, a Manual flow következő lépésre váltást, a contributor hozzáadás működését, és hogy üres contributor nem adható hozzá, valamint a Next gomb csak kitöltött projektnév esetén aktív.

\textbf{TasksView és SubTasksView komponensek}: Mockolt globális állapottal ellenőrzik, hogy a feladatok és alfeladatok cím szerint megjelennek-e a nézetben, illetve hogy kiválasztott projekt hiányában a megfelelő üzenet jelenik-e meg.

\textbf{Sidebar és Topbar}: Strukturális tesztek ellenőrzik, hogy a megfelelő számú gomb, a navigációs elemek és a várt HTML elem (\texttt{header} szerepkör) megjelenik-e.

\begin{table}[h!]
\centering
\caption{A tesztkészletek összefoglalása \cite{jest, vitest}}
...
\hline
auth.test.ts & Regisztráció, bejelentkezés, token validáció & 10 \\
projects.test.ts & CRUD, jogosultság, taskGroup import & 17 \\
tasks.test.ts & CRUD, állapotváltás, határidő kezelés & 10 \\
subtasks.test.ts & CRUD, alapértékek, XP mező & 10 \\
xpService.test.ts & XP hozzáadás, szintlépés, perzisztencia & 8 \\
githubService.test.ts & API hívások, adat transzformáció & 12 \\
logService.test.ts & Naplóbejegyzés mentése, userId kezelés & 2 \\
app.test.ts & Alapvető route ellenőrzés & 1 \\
\hline
\textbf{Backend összesen} & & \textbf{70} \\
\hline
Reducer.test.tsx & AppReducer action típusok & 10 \\
...
Actions.test.ts & fetchTasks, addTask, fetchSubtasks és társaik & 8 \\
Sign.test.tsx & Form render, submit, loading/error & 7 \\
CreateProject.test.tsx & Lépések, contributor kezelés & 6 \\
TasksView.test.tsx & Feladatlista renderelés & 2 \\
SubTasksView.test.tsx & Alfeladatlista renderelés & 2 \\
Sidebar.test.tsx & Ikonok száma, kattinthatóság & 2 \\
Topbar.test.tsx & Banner elem, navigációs gombok & 3 \\
\hline
\textbf{Frontend összesen} & & \textbf{47} \\
\hline
\textbf{Összes teszteset} & & \textbf{117} \\
\hline
\end{tabularx}
\end{table}
...
\caption{A frontend unit tesztek futtatásának eredménye \cite{vitest}}
\label{fig:test-vitest}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{images/jest_results.png}
\caption{A backend integrációs tesztek futtatásának eredménye \cite{jest}}
\label{fig:test-jest}
\end{figure}
...
A tesztlefedettség mérése a Jest, illetve a Vitest beépített coverage riportjával történt (\texttt{jest -{}-coverage} és \texttt{npx vitest -{}-coverage} parancsokkal).

\SubSection{Backend tesztlefedettség}

A backend esetén a 70 teszteset összesített eredménye: \textbf{61,33\% utasítás- (statement), 37,05\% ág- (branch), 51,11\% függvény- (function) és 62,67\% sorfedettség (line coverage)}. A rétegenkénti bontás a következőket mutatja:

\begin{itemize}
  \item \textbf{Modellek (src/models)}: 100\%-os lefedettség minden mérőszám szerint — a \texttt{Log}, \texttt{Mail}, \texttt{Project}, \texttt{Sprint}, \texttt{Subtask}, \texttt{Task} és \texttt{User} modellek teljes egészükben érintve vannak a tesztek által.
  \item \textbf{Service-ek (src/services)}: 97,36\% utasítás- és 96,77\% sorfedettség. Az \texttt{xpService} és a \texttt{githubService} 100\%-on van, a \texttt{logService} kisebb hiányossággal (85,71\% utasítás).
  \item \textbf{Middleware (src/middleware)}: 93,75\% utasítás- és sorfedettség, az auth middleware egyetlen lefedetlen sora a 35-ös sor.
  \item \textbf{Route-ok (src/routes)}: 55,61\% utasítás- és 57,64\% sorfedettség. Az \texttt{auth.ts} (93,54\%) és a \texttt{projects.ts} (84,05\%) jól lefedett, míg a \texttt{mails.ts} (22,22\%), \texttt{users.ts} (21,62\%) és \texttt{sprints.ts} (27,27\%) route-ok teszteltsége alacsony — ezek felelnek a globálisan alacsonyabb értékekért.
  \item \textbf{Controller (src/controllers)}: A \texttt{githubController.ts} teljes utasításfedettége 20,58\%, sorfedettége 14,28\%; a GitHub webhook-kezelő logika nagy része nem tesztelt.
\end{itemize}

\SubSection{Frontend tesztlefedettség}

A frontend esetén a Vitest lefedettségmérése az összes 11 tesztfájlra és 47 tesztesetre kiterjedt. A konkrét sorlefedettségi számot a \texttt{--coverage} kapcsolóval futtatott mérés adja meg; a tesztek a következő területeket fedik le: \texttt{Sign}, \texttt{Topbar}, \texttt{Sidebar}, \texttt{CreateProject} komponensek, a \texttt{TasksView} és \texttt{SubTasksView} nézetek, valamint a \texttt{GlobalContext}, \texttt{ViewContext}, \texttt{Reducer}, \texttt{ViewReducer} és \texttt{Actions} állapotkezelő egységek. A mobil alkalmazás (React Native/Expo) frontend komponensei nem kerültek unit tesztelésre, ami a frontend összesített lefedettségi értékét érdemben csökkenti.

A lefedettségmérés automatizálása CI/CD pipeline keretében a jövőbeli fejlesztések egyik lehetséges iránya.
