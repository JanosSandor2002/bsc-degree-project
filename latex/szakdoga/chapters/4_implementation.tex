...
A modellek TypeScript interfészeket definiálnak \cite{typescript} (\texttt{IUser}, \texttt{IProject} stb.), amelyek lehetővé teszik a típusbiztos hozzáférést a dokumentum mezőkhöz. Az alábbiakban a \texttt{User} modell megvalósításának főbb részei láthatók:
...
\begin{lstlisting}[caption={Webes frontend könyvtárstruktúra}]
frontend/
  src/
...
A \texttt{GlobalContext} a \texttt{useReducer} hook segítségével kezel egy komplex állapotobjektumot. Az \texttt{AppReducer} minden lehetséges action típusra tartalmaz egy ágat, és mindig új állapotobjektumot ad vissza (immutable update). Az \texttt{Actions.ts} fájl aszinkron action creator függvényeket tartalmaz, amelyek axios segítségével hívják az API-t, és dispatch hívásokkal frissítik a globális állapotot.
...
\begin{lstlisting}[language=JavaScript, caption={Egyedi tab bar implementáció (layout.tsx)}]
...
A mobil alkalmazás főbb nézetei a \ref{fig:impl-mobile}. ábrán láthatók.

\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{images/mobile_screens.jpeg}
\caption{A mobil alkalmazás főbb nézetei}
\label{fig:impl-mobile}
\end{figure}

\Section{Egységtesztek implementációja}


A rendszer tesztelése két szinten valósul meg: backend oldalon Jest \cite{jest} és Supertest segítségével írt integrációs tesztek, frontend oldalon Vitest \cite{vitest} és React Testing Library alapú unit tesztek.

\SubSection{Backend tesztelés}

A backend tesztek in-memory MongoDB adatbázist alkalmaznak (\texttt{mongodb-\allowbreak{}memory-\allowbreak{}server}), ami azt jelenti, hogy a tesztek nem igényelnek élő adatbázis-kapcsolatot, és teljesen izoláltan futnak. A \texttt{setup.ts} fájl minden tesztfájl előtt elindítja a memóriaadatbázist, és minden egyes teszt után törli az összes kollekcióból az adatokat, hogy biztosítsa az izolált futtatást.
...
Az integrációs tesztek a Supertest könyvtár segítségével közvetlenül az Express alkalmazásnak küldenek HTTP kéréseket, és ellenőrzik a visszakapott státuszkódot, a válasz testét és az adatbázis állapotát. A tesztek lefedik az auth, project, task, subtask és service szintű működést. A futtatás eredménye a \ref{fig:impl-jest}. ábrán látható.

\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{images/jest_tests.png}
\caption{A backend tesztek futtatásának eredménye \cite{jest}}
\label{fig:impl-jest}
\end{figure}

\SubSection{Frontend tesztelés}

A webes frontend tesztjei Vitest \cite{vitest} és React Testing Library segítségével készültek. A tesztek négy fő területet fednek le:
...
\caption{A frontend tesztek futtatásának eredménye \cite{vitest}}
\label{fig:impl-vitest}
\end{figure}

\SubSection{GitHub service tesztelés}

A \texttt{githubService.ts} tesztelése érdekes kihívást jelentett, mivel a GitHub API-t nem szerettük volna valóban hívni a tesztek során. Az axios mockolt verziójával (\texttt{jest.\allowbreak{}mock(\allowbreak{}'axios')}) a tesztek ellenőrzik, hogy a service függvények a helyes URL-t hívják-e, a helyes headereket küldik-e el, és a válasz adatait a várt formátumra alakítják-e. A \texttt{logService.ts} tesztelésénél a \texttt{console.log} spy-olásával ellenőriztük, hogy a naplóbejegyzések a várt formátumban kerülnek kiírásra.

\Section{Fejlesztői és tesztkörnyezet}


A rendszer fejlesztése és futtatása a következő környezetben történt, amelynek ismerete szükséges az eredmények reprodukálásához.

\textbf{Szerveroldal}: Node.js 20 LTS, TypeScript 5, MongoDB 7 (lokálisan futtatva vagy MongoDB Atlas felhőszolgáltatáson keresztül). A backend indítása: \texttt{npm run dev} (ts-node-dev-vel), a tesztek futtatása: \texttt{npm test}.

\textbf{Webes frontend}: Node.js 20 LTS, Vite 5, React 18. Fejlesztői szerver indítása: \texttt{npm run dev} (alapértelmezetten a \texttt{http://localhost:5173} címen érhető el). A tesztek futtatása: \texttt{npm run test} (Vitest).

\textbf{Mobil alkalmazás}: Expo SDK 51, React Native 0.74. Előfeltétel az Expo CLI telepítése (\texttt{npm install -g expo-cli}), majd az alkalmazás indítható \texttt{npx expo start} paranccsal. Fizikai eszközön az Expo Go alkalmazáson keresztül futtatható, a fejlesztői gép és a mobileszköz azonos LAN hálózaton legyen.

\textbf{Szükséges környezeti változók} (backend \texttt{.env} fájl):
\begin{itemize}
  \item \texttt{MONGO\_URI} -- MongoDB kapcsolati string,
  \item \texttt{JWT\_SECRET} -- JWT aláíráshoz használt titkos kulcs,
  \item \texttt{PORT} -- szerver portja (alapértelmezett: \texttt{5000}).
\end{itemize}
...
\textbf{Sprint törlés és feladat konzisztencia}: Jelenleg sprint törlésekor a hozzá rendelt feladatok \texttt{sprint} mezője nem törlődik automatikusan a backendben, ami konzisztencia-problémát okozhat. Ezt egy MongoDB aggregációs pipeline-nal vagy Mongoose \texttt{mid\-dle\-ware}-rel lehetne kezelni.
